...
open class OverscrollableNestedScrollView : NestedScrollView {
    ...
    open fun isAchieveTop() = scrollY <= 0

    private fun getBaseY() = (y - translationY).toInt()
    ...
    override fun dispatchTouchEvent(ev: MotionEvent): Boolean =
        when (ev.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (headerView == null) {
                    if ((lastScrollY > 0) && lastYDelta < 0 && isAchieveTop()) {
                        initScrollAction(ev)
                        false
                    } else if (lastYDelta < 0 && isAchieveTop()) {
                        pullDown(lastYDelta)
                        false
                    } else {
                        super.dispatchTouchEvent(ev)
                    ...
                    if ((lastScrollY > 0 || lastHeaderYPos < 0) && lastYDelta < 0 && getBaseY() == startHeaderSize) {
                        initScrollAction(ev)
                        true
                    } else if (lastYDelta < 0 && getBaseY() == (headerView?.measuredHeight ?: 0)) {
                        pullDown(lastYDelta)
                        false
                    } else {
                        super.dispatchTouchEvent(ev)
